...
data](https://docs.ropensci.org/dittodb/articles/nycflights.md)[^1]. We
...
#> ── Warning: mean_delays() ──────────────────────────────────────────────────────
#> Can't find generic `db_supports_table_alias_with_as` in package dbplyr to register S3 method.
#> i This message is only shown to developers using devtools.
#> i Do you need to update dbplyr to the latest version?
#> Backtrace:
#>     ▆
#>  1. └─global mean_delays("month")
#>  2.   └─DBI::dbConnect(RMariaDB::MariaDB(), dbname = "nycflights")
#>  3.     ├─base::.doTrace(...)
#>  4.     │ └─base::eval.parent(exprObj)
#>  5.     │   └─base::eval(expr, p)
#>  6.     │     └─base::eval(expr, p)
#>  7.     └─dittodb::dbMockConnect(drv, ...)
#>  8.       └─dittodb:::s3_register(...)
#>  9.         └─dittodb (local) register()
...
Use the function
[`dittodb::use_dittodb()`](https://docs.ropensci.org/dittodb/reference/use_dittodb.md)
...
[^1]: These code snippets won’t work right out of the box — you will
    need to make sure the database you’re connecting to has the
    [nycflights13
    data](https://docs.ropensci.org/dittodb/articles/nycflights.md) data
    in it. You could do this with SQLite by running
